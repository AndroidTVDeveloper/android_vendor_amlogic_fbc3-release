...
void cmd_reg_bits_write(char *arg_addr,
			char *arg_bit, char *arg_width, char *arg_val)
{
...
int print_buffer(unsigned addr, void *data,
		 unsigned width, unsigned count, unsigned linelen)
{
	/* linebuf as a union causes proper alignment */
	union linebuf {
		unsigned ui[MAX_LINE_LENGTH_BYTES / sizeof(unsigned) + 1];
		unsigned short us[MAX_LINE_LENGTH_BYTES /
				  sizeof(unsigned short) + 1];
		unsigned char uc[MAX_LINE_LENGTH_BYTES /
