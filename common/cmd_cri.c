	...
	if (strcmp(cmd, "pid") == 0) {
		...
		if (strcmp(cmd, "r") == 0) {
			...
			if (select_project_id(tmp_val) == 0)
				printf
				    ("save pro_id %d,load pqok.\n",
				     tmp_val, tmp_val);
			else
				printf
				    ("save pro_id %d,load pqerror!\n",
				     tmp_val, tmp_val);
			return 0;
		}
		...
		if (strcmp(cmd, "r") == 0) {
			...
			spi_flash_read(get_spi_flash_device(0), tmp_off,
				       tmp_len, tmp_buf);
			for (i = 0; i < tmp_len; i++) {
				if (i % 16 == 0)
					printf("\n%08X:", tmp_off + i);
			...
			if (tmp_buf != NULL) {
				...
				for (i = 0; i < tmp_len / 2; i++) {
					...
					tmp_buf[i] =
					    strtoul((const char *)tmp_chs,
						    &endp, 16);
				}

				if (tmp_len !=
				    spi_flash_random_write(get_spi_flash_device
							   (0), tmp_off,
							   tmp_len / 2,
							   tmp_buf) * 2) {
			...
			if (tmp_buf != NULL) {
				...
				if (tmp_len !=
				    spi_flash_random_write(get_spi_flash_device
							   ...
							   tmp_len, tmp_buf)) {
					printf
				    ("spi wr 0x%02X(0x%08X,0x%08X) fail!\n",
				     tmp_val, tmp_off, tmp_len);
				} else
					printf
				    ("spi erase 0x%02X(0x%08X,0x%08X) ok\n",
				     tmp_val, tmp_off, tmp_len);
