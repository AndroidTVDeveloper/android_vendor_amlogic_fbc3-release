...
static int fbc_version_info_init(void)
{
	...
	version_serial =
	(long long)(FBC_VERSION_MAIN & 0xff) << 56 | (long
		long)(FBC_VERSION_SUB1
		& 0xff) << 48 |
		(long long)(FBC_VERSION_SUB2 & 0xff) << 32 | shortgitversion;
	sprintf(versioninfo, "%d.%d.%d.%s", FBC_VERSION_MAIN,
		FBC_VERSION_SUB1, FBC_VERSION_SUB2, git_shor_version);
	return 0;
}
