...
static unsigned int handle_dbg_cmd(unsigned char *s, int *rets)
{
	...
	case CMD_COMMUNICATION_TEST:
		...
		for (i = 0; i < tmp_val; i++) {
			tmp_ch = s[i + 1];
			tmp_ch =
			    ((tmp_ch & 0x55) << 1) | ((tmp_ch & 0xAA) >> 1);
			tmp_ch =
			    ((tmp_ch & 0x33) << 2) | ((tmp_ch & 0xCC) >> 2);
			tmp_ch =
			    ((tmp_ch & 0x0F) << 4) | ((tmp_ch & 0xF0) >> 4);
			rets[i] = tmp_ch;
		}
		break;
	case CMD_CLR_SETTINGS_DEFAULT:
	...
	case CMD_DBG_SPI_ACCESS:
		...
		if (params != NULL) {
			...
			spi_flash_read(get_spi_flash_device(0), offset,
				       size * 4, (unsigned *)((unsigned)rets));
...
void dbg_task_init(void)
{
	...
	RegisterCmd(&dbg_list, dbg_task_id,
		    INPUT_CEC | INPUT_UART_HOST | INPUT_UART_CONSOLE,
		    check_cmd_is_supported, handle_dbg_cmd);
	return;
}
