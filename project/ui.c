...
struct menu_item_t  menu_screen = { 0, -1, "main menu 1234567890",
	MENU_START_LINE + 0,
	MENU_START_POSITION + 0, NULL, NULL,
	&menu_test1, NULL, &process_menu,
	NULL
};

struct menu_item_t  menu_test1 = { 1, -1, "HV FLIP",
	MENU_START_LINE + 1, MENU_START_POSITION + 128,
	&menu_mute, &menu_test2, NULL,
	&menu_screen, &process_menu, &do_test1
};

struct menu_item_t  menu_test2 = { 2, -1, "Show logo",
	MENU_START_LINE + 2, MENU_START_POSITION + 128,
	&menu_test1, &menu_brightness, NULL,
	&menu_screen, &process_menu,
	&do_test2
};

struct menu_item_t  menu_brightness = { 3, -1, "adjust brightness",
	MENU_START_LINE + 3, MENU_START_POSITION + 128,
	&menu_test2, &menu_contrast, NULL,
	&menu_screen, &process_menu,
	NULL
};

struct menu_item_t  menu_contrast = { 4, -1, "adjust contrast",
	MENU_START_LINE + 4, MENU_START_POSITION + 128,
	&menu_brightness, &menu_volume, NULL,
	&menu_screen, &process_menu, NULL
};

struct menu_item_t  menu_volume = { 5, -1, "adjust volume",
	MENU_START_LINE + 5, MENU_START_POSITION + 128,
	&menu_contrast, &menu_mute, NULL,
	&menu_screen, &process_menu, NULL
};

struct menu_item_t  menu_mute = { 6, -1, "mute", MENU_START_LINE + 6,
	MENU_START_POSITION + 128,
	&menu_volume, &menu_test1,
	NULL, &menu_screen, &process_menu, NULL
};
	...
	if (plist) {
		...
		if (input_list) {
			...
			if (current_menu_item != NULL) {
				return
				    current_menu_item->key_process
				    (current_menu_item, cmd);
			}
		}
	}
	return 0;
...
#define DISABLE_MENU_TOD0
	...
	case AMLKEY_ENTER:
		if (item->id == menu_brightness.id
		    || item->id == menu_contrast.id
		    || item->id == menu_volume.id || item->id == menu_mute.id)
			break;
...
void invalidateValue(struct menu_item_t *item, int key)
{
	...
	if (item->id == menu_brightness.id) {
		curValue =
		    RunCommand(VPU_CMD_BRIGHTNESS | VPU_CMD_READ, NULL)[0];
	...
	if (menu_item_value[item->id].handle > 0)
		OSD_UpdateRegionContentSimple(menu_item_value[item->id].handle,
					      toDisplayvalue,
					      key !=
					      NULL ? FONT_HILIGHT_COLOR_INDEX :
					      FONT_NORMAL_COLOR_INDEX,
					      FONT_BG_COLOR_INDEX);
}
...
void invalidate(struct menu_item_t *last, struct menu_item_t *curr)
{
	...
	if (curr->id == menu_screen.id) {
		LOGD(TAG,
		     "disable osd fresh curr item id is %d, last id is %d.\n",
		     curr->id, last->id);
		hide_menu();
		...
		show_menu();
		LOGD(TAG,
		     "enable osd fresh curr item id is %d, last id is %d.\n",
		     curr->id, last->id);
		...
		if (last->handle < 0) {
			last->handle =
			    OSD_InitialRegionSimple(last->line, 0, last->name,
						    FONT_NORMAL_COLOR_INDEX,
						    FONT_BG_COLOR_INDEX);
			...
			OSD_UpdateRegionContentSimple(last->handle, NULL,
						      FONT_NORMAL_COLOR_INDEX,
						      FONT_BG_COLOR_INDEX);
			OSD_UpdateRegionContentSimple(menu_item_value
						      [last->id].handle, NULL,
						      FONT_NORMAL_COLOR_INDEX,
						      FONT_BG_COLOR_INDEX);
		}
		if (curr->handle < 0) {
			curr->handle =
			    OSD_InitialRegionSimple(curr->line, 0, curr->name,
						    FONT_HILIGHT_COLOR_INDEX,
						    FONT_BG_COLOR_INDEX);
			...
			OSD_UpdateRegionContentSimple(curr->handle, NULL,
						      FONT_HILIGHT_COLOR_INDEX,
						      FONT_BG_COLOR_INDEX);
			OSD_UpdateRegionContentSimple(menu_item_value
						      [curr->id].handle, NULL,
...
void show_menu(void)
{
	...
	if (menu_screen.handle < 0)
		menu_screen.handle =
		    OSD_InitialRegionSimple(menu_screen.line, menu_screen.col,
					    menu_screen.name,
					    FONT_NORMAL_COLOR_INDEX,
					    FONT_BG_COLOR_INDEX);
	if (menu_test1.handle < 0)
		menu_test1.handle =
		    OSD_InitialRegionSimple(menu_test1.line, menu_test1.col,
					    menu_test1.name,
					    FONT_HILIGHT_COLOR_INDEX,
					    FONT_BG_COLOR_INDEX);
	if (menu_test2.handle < 0)
		menu_test2.handle =
		    OSD_InitialRegionSimple(menu_test2.line, menu_test2.col,
					    menu_test2.name,
					    FONT_NORMAL_COLOR_INDEX,
					    FONT_BG_COLOR_INDEX);
	if (menu_brightness.handle < 0)
		menu_brightness.handle =
		    OSD_InitialRegionSimple(menu_brightness.line,
					    menu_brightness.col,
					    menu_brightness.name,
					    FONT_NORMAL_COLOR_INDEX,
					    FONT_BG_COLOR_INDEX);
	if (menu_contrast.handle < 0)
		menu_contrast.handle =
		    OSD_InitialRegionSimple(menu_contrast.line,
					    menu_contrast.col,
					    menu_contrast.name,
					    FONT_NORMAL_COLOR_INDEX,
					    FONT_BG_COLOR_INDEX);
	if (menu_volume.handle < 0)
		menu_volume.handle =
		    OSD_InitialRegionSimple(menu_volume.line, menu_volume.col,
					    menu_volume.name,
					    FONT_NORMAL_COLOR_INDEX,
					    FONT_BG_COLOR_INDEX);
	if (menu_mute.handle < 0)
		menu_mute.handle =
		    OSD_InitialRegionSimple(menu_mute.line, menu_mute.col,
					    menu_mute.name,
					    FONT_NORMAL_COLOR_INDEX,
					    FONT_BG_COLOR_INDEX);

	char *menu_name[7] = { menu_screen.name, menu_test1.name,
		menu_test2.name, menu_brightness.name,
		menu_contrast.name, menu_volume.name,
		menu_mute.name
	};
	for (int i = 0;
	     i < sizeof(menu_item_value) / sizeof(menu_item_value[0]); i++) {
		if (menu_item_value[i].handle < 0)
			menu_item_value[i].handle =
			    OSD_InitialRegionSimple(menu_item_value[i].line,
						    menu_item_value[i].col +
						    700,
						    menu_item_value[i].name,
						    i ==
						    1 ? FONT_HILIGHT_COLOR_INDEX
						    : FONT_NORMAL_COLOR_INDEX,
						    FONT_BG_COLOR_INDEX);
	}
...
void init_ui(void)
{
	...
	RegisterInput(&osd_input_data_list, osd_task_id,
		      INPUT_REMOTE | INPUT_SARADC, NULL);
	...
	if (UiGetHaveLogoFlag() == 1) {
		init_logo_osd();
		logo_task_id =
		    RegisterTask(logo_task_handle, NULL, 0, TASK_PRIORITY_OSD);
...
int KeyFunc(unsigned int input_type, unsigned int customer_code,
