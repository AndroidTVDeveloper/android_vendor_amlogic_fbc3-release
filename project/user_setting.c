...
static struct cri_data gCriData = {
	.prj_id = {
		   .prj_id_ori = 0,
		   .prj_id_chksum = 0,
		   .reversed = "",
...
unsigned read_project_id(void)
{
	...
	if (isPorjectIDValid(gCriData.prj_id.prj_id_ori) == 0) {
		...
		tmp_crc =
		    crc32(0, (unsigned char *)&gCriData.prj_id.prj_id_ori,
			  sizeof(unsigned int));
		...
		if (tmp_crc == gCriData.prj_id.prj_id_chksum) {
			...
			printf
			    ("(%d, 0x%08X, 0x%08X)!\n",
			     gCriData.prj_id.prj_id_ori, tmp_crc,
			     gCriData.prj_id.prj_id_chksum);
...
int write_project_id(unsigned val)
{
	...
	if (isPorjectIDValid(val) == 0) {
		...
		tmp_id.prj_id_chksum =
		    crc32(0, (unsigned char *)&val, sizeof(unsigned int));

		tmp_offset = 0;

		if (spi_flash_random_write
		    (get_spi_flash_device(0), USER_CRI_DATA_START + tmp_offset,
		     sizeof(struct cri_prj_id), &tmp_id) > 0) {
...
char *read_device_id()
{
	...
	tmp_crc =
	    crc32(0, (unsigned char *)&gCriData.dev_id.dev_id_len,
		  sizeof(unsigned int));
	...
	if (tmp_crc == gCriData.dev_id.dev_id_l_chksum) {
		...
		if (gCriData.dev_id.dev_id_len < CC_CRI_DEVICE_ID_LEN) {
			...
			tmp_crc =
			    crc32(0, gCriData.dev_id.dev_id_buf,
				  gCriData.dev_id.dev_id_len);
		...
		printf
		    ("(%d, 0x%08X, 0x%08X)!\n",
		     gCriData.dev_id.dev_id_len, tmp_crc,
		     gCriData.dev_id.dev_id_l_chksum);
...
int write_device_id(char *device)
{
	...
	if (tmp_len < CC_CRI_DEVICE_ID_LEN) {
		...
		tmp_id.dev_id_l_chksum =
		    crc32(0, (unsigned char *)&tmp_id.dev_id_len,
			  sizeof(unsigned int));

		strcpy(tmp_id.dev_id_buf, device);

		tmp_id.dev_id_b_chksum =
		    crc32(0, tmp_id.dev_id_buf, tmp_id.dev_id_len);

		tmp_offset = sizeof(struct cri_prj_id) + sizeof(struct cri_rev);

		if (spi_flash_random_write
		    (get_spi_flash_device(0), USER_CRI_DATA_START + tmp_offset,
		     sizeof(struct cri_dev_id), &tmp_id) > 0) {
		...
		memcpy((void *)&g_user_setting_save.system,
		       (void *)g_user_setting.system, sizeof(struct systems));

		if (spi_flash_random_write
		    (get_spi_flash_device(0), USER_SETTING_START,
		     sizeof(struct user_setting_save),
		     &g_user_setting_save) > 0) {
...
char *read_factory_sn()
{
	...
	tmp_crc =
	    crc32(0, (unsigned char *)&gCriData.fac_sn.fac_sn_len,
		  sizeof(unsigned int));
	...
	if (tmp_crc == gCriData.fac_sn.fac_sn_l_chksum) {
		...
		if (gCriData.fac_sn.fac_sn_len < CC_CRI_FAC_SN_LEN) {
			...
			tmp_crc =
			    crc32(0, gCriData.fac_sn.fac_sn_buf,
				  gCriData.fac_sn.fac_sn_len);
		...
		printf
		    ("(%d, 0x%08X, 0x%08X)!\n",
		     gCriData.fac_sn.fac_sn_len, tmp_crc,
		     gCriData.fac_sn.fac_sn_l_chksum);
...
int write_factory_sn(char *pSn)
{
	...
	if (tmp_len < CC_CRI_FAC_SN_LEN) {
		...
		tmp_sn.fac_sn_l_chksum =
		    crc32(0, (unsigned char *)&tmp_sn.fac_sn_len,
			  sizeof(unsigned int));

		strcpy(tmp_sn.fac_sn_buf, pSn);

		tmp_sn.fac_sn_b_chksum =
		    crc32(0, tmp_sn.fac_sn_buf, tmp_sn.fac_sn_len);

		tmp_offset =
		    sizeof(struct cri_prj_id) + sizeof(struct cri_rev) +
		    sizeof(struct cri_dev_id);

		if (spi_flash_random_write
		    (get_spi_flash_device(0), USER_CRI_DATA_START + tmp_offset,
		     sizeof(struct cri_fac_sn), &tmp_sn) > 0) {
		...
		if (tmp_len < CC_FACTORY_SN_SIZE) {
			...
			memcpy((void *)&g_user_setting_save.system,
			       (void *)g_user_setting.system,
			       sizeof(struct systems));
			...
			if (spi_flash_random_write
				...
				printf
				    ("system factory sn \"%s\" save is OK!\n",
...
char *read_HDCP_KEY()
{
	...
	if (spi_flash_read
	    (get_spi_flash_device(0), USER_HDCPKEY, USER_HDCPKEY_SIZE,
	     (void *)key) <= 0) {
...
int write_HDCP_KEY(const char *key, int len)
{
	...
	int ret = spi_flash_random_write(get_spi_flash_device(0), USER_HDCPKEY,
					 USER_HDCPKEY_SIZE, (void *)p);
...
white_balance_setting_t *read_wb_setting(vpu_colortemp_t mode)
{
	...
	white_balance_setting_t *val =
	    (white_balance_setting_t *) malloc(sizeof(white_balance_setting_t));
...
#ifdef CONFIG_RANDOM_WRITE
	return spi_flash_random_write(get_spi_flash_device(0),
				      USER_WB_SETTING_START,
				      sizeof(struct wb_setting), &g_wb_setting);
...
int write_picmod_setting(vpu_picmod_t mode, vpu_picmod_table_t *val)
{
	...
	memcpy((void *)&picmod_table[mode], (void *)val,
	       sizeof(vpu_picmod_table_t));

	return spi_flash_random_write(get_spi_flash_device(0),
				      USER_PIC_SETTING_START,
				      PICMOD_MAX * sizeof(vpu_picmod_table_t),
				      &picmod_table);
...
int load_default_user_setting(int clr_version_flag)
{
	...
	spi_flash_read(get_spi_flash_device(0), USER_SETTING_START,
		       sizeof(struct user_setting_save),
		       (void *)&tmp_usr_setting_save);

	memcpy((void *)&g_user_setting_save.system, (void *)&system_default,
	       sizeof(struct systems));

	memcpy((void *)&g_user_setting_save.system.factory_sn,
	       (void *)&tmp_usr_setting_save.system.factory_sn,
	       CC_FACTORY_SN_SIZE);
	...
	memcpy((void *)&g_user_setting_save.pq, (void *)&pq_def,
	       sizeof(vpu_fac_pq_t));

	/* load default audio data */
	memcpy((void *)&g_user_setting_save.audio, (void *)&audio_state_def,
	       sizeof(struct audio_control));
...
int load_default_wb_setting(void)
{
	...
	if (WB_DATA_FROM_DB) {
		...
		g_wb_setting.cold.wb_param.gain_r =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[2].gain_r, 256, 2048, 0);

		g_wb_setting.cold.wb_param.gain_g =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[2].gain_g, 256, 2048, 0);

		g_wb_setting.cold.wb_param.gain_b =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[2].gain_b, 256, 2048, 0);

		g_wb_setting.cold.wb_param.pre_offset_r =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[2].pre_offset_r, 256,
						    2048, 1);

		g_wb_setting.cold.wb_param.pre_offset_g =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[2].pre_offset_g, 256,
						    2048, 1);

		g_wb_setting.cold.wb_param.pre_offset_b =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[2].pre_offset_b, 256,
						    2048, 1);

		g_wb_setting.cold.wb_param.post_offset_r =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[2].post_offset_r, 256,
						    2048, 1);

		g_wb_setting.cold.wb_param.post_offset_g =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[2].post_offset_g, 256,
						    2048, 1);

		g_wb_setting.cold.wb_param.post_offset_b =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[2].post_offset_b, 256,
						    2048, 1);

		g_wb_setting.standard.color_temp = COLOR_TEMP_STD;

		g_wb_setting.standard.wb_param.gain_r =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[0].gain_r, 256, 2048, 0);

		g_wb_setting.standard.wb_param.gain_g =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[0].gain_g, 256, 2048, 0);

		g_wb_setting.standard.wb_param.gain_b =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[0].gain_b, 256, 2048, 0);

		g_wb_setting.standard.wb_param.pre_offset_r =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[0].pre_offset_r, 256,
						    2048, 1);

		g_wb_setting.standard.wb_param.pre_offset_g =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[0].pre_offset_g, 256,
						    2048, 1);

		g_wb_setting.standard.wb_param.pre_offset_b =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[0].pre_offset_b, 256,
						    2048, 1);

		g_wb_setting.standard.wb_param.post_offset_r =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[0].post_offset_r, 256,
						    2048, 1);

		g_wb_setting.standard.wb_param.post_offset_g =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[0].post_offset_g, 256,
						    2048, 1);

		g_wb_setting.standard.wb_param.post_offset_b =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[0].post_offset_b, 256,
						    2048, 1);

		g_wb_setting.warm.color_temp = COLOR_TEMP_WARM;

		g_wb_setting.warm.wb_param.gain_r =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[1].gain_r, 256, 2048, 0);

		g_wb_setting.warm.wb_param.gain_g =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[1].gain_g, 256, 2048, 0);

		g_wb_setting.warm.wb_param.gain_b =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[1].gain_b, 256, 2048, 0);

		g_wb_setting.warm.wb_param.pre_offset_r =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[1].pre_offset_r, 256,
						    2048, 1);

		g_wb_setting.warm.wb_param.pre_offset_g =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[1].pre_offset_g, 256,
						    2048, 1);

		g_wb_setting.warm.wb_param.pre_offset_b =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[1].pre_offset_b, 256,
						    2048, 1);

		g_wb_setting.warm.wb_param.post_offset_r =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[1].post_offset_r, 256,
						    2048, 1);

		g_wb_setting.warm.wb_param.post_offset_g =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[1].post_offset_g, 256,
						    2048, 1);

		g_wb_setting.warm.wb_param.post_offset_b =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[1].post_offset_b, 256,
						    2048, 1);

		g_wb_setting.user.color_temp = COLOR_TEMP_USER;

		g_wb_setting.user.wb_param.gain_r =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[0].gain_r, 256, 2048, 0);

		g_wb_setting.user.wb_param.gain_g =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[0].gain_g, 256, 2048, 0);

		g_wb_setting.user.wb_param.gain_b =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[0].gain_b, 256, 2048, 0);

		g_wb_setting.user.wb_param.pre_offset_r =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[0].pre_offset_r, 256,
						    2048, 1);

		g_wb_setting.user.wb_param.pre_offset_g =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[0].pre_offset_g, 256,
						    2048, 1);

		g_wb_setting.user.wb_param.pre_offset_b =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[0].pre_offset_b, 256,
						    2048, 1);

		g_wb_setting.user.wb_param.post_offset_r =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[0].post_offset_r, 256,
						    2048, 1);

		g_wb_setting.user.wb_param.post_offset_g =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[0].post_offset_g, 256,
						    2048, 1);

		g_wb_setting.user.wb_param.post_offset_b =
		    (unsigned short)pq_data_mapping((unsigned int)
						    vpu_config_table.
						    wb[0].post_offset_b, 256,
						    2048, 1);
		...
		g_wb_setting.cold.wb_param.gain_r =
		    colortemp_table[COLOR_TEMP_COLD].wb_param.gain_r;

		g_wb_setting.cold.wb_param.gain_g =
		    colortemp_table[COLOR_TEMP_COLD].wb_param.gain_g;

		g_wb_setting.cold.wb_param.gain_b =
		    colortemp_table[COLOR_TEMP_COLD].wb_param.gain_b;

		g_wb_setting.cold.wb_param.pre_offset_r =
		    colortemp_table[COLOR_TEMP_COLD].wb_param.pre_offset_r;

		g_wb_setting.cold.wb_param.pre_offset_g =
		    colortemp_table[COLOR_TEMP_COLD].wb_param.pre_offset_g;

		g_wb_setting.cold.wb_param.pre_offset_b =
		    colortemp_table[COLOR_TEMP_COLD].wb_param.pre_offset_b;

		g_wb_setting.cold.wb_param.post_offset_r =
		    colortemp_table[COLOR_TEMP_COLD].wb_param.post_offset_r;

		g_wb_setting.cold.wb_param.post_offset_g =
		    colortemp_table[COLOR_TEMP_COLD].wb_param.post_offset_g;

		g_wb_setting.cold.wb_param.post_offset_b =
		    colortemp_table[COLOR_TEMP_COLD].wb_param.post_offset_b;

		g_wb_setting.standard.color_temp = COLOR_TEMP_STD;

		g_wb_setting.standard.wb_param.gain_r =
		    colortemp_table[COLOR_TEMP_STD].wb_param.gain_r;

		g_wb_setting.standard.wb_param.gain_g =
		    colortemp_table[COLOR_TEMP_STD].wb_param.gain_g;

		g_wb_setting.standard.wb_param.gain_b =
		    colortemp_table[COLOR_TEMP_STD].wb_param.gain_b;

		g_wb_setting.standard.wb_param.pre_offset_r =
		    colortemp_table[COLOR_TEMP_STD].wb_param.pre_offset_r;

		g_wb_setting.standard.wb_param.pre_offset_g =
		    colortemp_table[COLOR_TEMP_STD].wb_param.pre_offset_g;

		g_wb_setting.standard.wb_param.pre_offset_b =
		    colortemp_table[COLOR_TEMP_STD].wb_param.pre_offset_b;

		g_wb_setting.standard.wb_param.post_offset_r =
		    colortemp_table[COLOR_TEMP_STD].wb_param.post_offset_r;

		g_wb_setting.standard.wb_param.post_offset_g =
		    colortemp_table[COLOR_TEMP_STD].wb_param.post_offset_g;

		g_wb_setting.standard.wb_param.post_offset_b =
		    colortemp_table[COLOR_TEMP_STD].wb_param.post_offset_b;

		g_wb_setting.warm.color_temp = COLOR_TEMP_WARM;

		g_wb_setting.warm.wb_param.gain_r =
		    colortemp_table[COLOR_TEMP_WARM].wb_param.gain_r;

		g_wb_setting.warm.wb_param.gain_g =
		    colortemp_table[COLOR_TEMP_WARM].wb_param.gain_g;

		g_wb_setting.warm.wb_param.gain_b =
		    colortemp_table[COLOR_TEMP_WARM].wb_param.gain_b;

		g_wb_setting.warm.wb_param.pre_offset_r =
		    colortemp_table[COLOR_TEMP_WARM].wb_param.pre_offset_r;

		g_wb_setting.warm.wb_param.pre_offset_g =
		    colortemp_table[COLOR_TEMP_WARM].wb_param.pre_offset_g;

		g_wb_setting.warm.wb_param.pre_offset_b =
		    colortemp_table[COLOR_TEMP_WARM].wb_param.pre_offset_b;

		g_wb_setting.warm.wb_param.post_offset_r =
		    colortemp_table[COLOR_TEMP_WARM].wb_param.post_offset_r;

		g_wb_setting.warm.wb_param.post_offset_g =
		    colortemp_table[COLOR_TEMP_WARM].wb_param.post_offset_g;

		g_wb_setting.warm.wb_param.post_offset_b =
		    colortemp_table[COLOR_TEMP_WARM].wb_param.post_offset_b;

		g_wb_setting.user.color_temp = COLOR_TEMP_USER;

		g_wb_setting.user.wb_param.gain_r =
		    colortemp_table[COLOR_TEMP_USER].wb_param.gain_r;

		g_wb_setting.user.wb_param.gain_g =
		    colortemp_table[COLOR_TEMP_USER].wb_param.gain_g;

		g_wb_setting.user.wb_param.gain_b =
		    colortemp_table[COLOR_TEMP_USER].wb_param.gain_b;

		g_wb_setting.user.wb_param.pre_offset_r =
		    colortemp_table[COLOR_TEMP_USER].wb_param.pre_offset_r;

		g_wb_setting.user.wb_param.pre_offset_g =
		    colortemp_table[COLOR_TEMP_USER].wb_param.pre_offset_g;

		g_wb_setting.user.wb_param.pre_offset_b =
		    colortemp_table[COLOR_TEMP_USER].wb_param.pre_offset_b;

		g_wb_setting.user.wb_param.post_offset_r =
		    colortemp_table[COLOR_TEMP_USER].wb_param.post_offset_r;

		g_wb_setting.user.wb_param.post_offset_g =
		    colortemp_table[COLOR_TEMP_USER].wb_param.post_offset_g;

		g_wb_setting.user.wb_param.post_offset_b =
		    colortemp_table[COLOR_TEMP_USER].wb_param.post_offset_b;
...
int load_default_picmod_setting(void)
{
	...
	memcpy((void *)&picmod_table, (void *)&pic_table_def,
	       PICMOD_MAX * sizeof(vpu_picmod_table_t));
...
static void load_user_setting(void)
{
	...
	memcpy((void *)g_user_setting.system,
	       (void *)&g_user_setting_save.system, sizeof(struct systems));

	memcpy((void *)g_user_setting.pq, (void *)&g_user_setting_save.pq,
	       sizeof(vpu_fac_pq_t));

	memcpy((void *)g_user_setting.audio,
	       (void *)&g_user_setting_save.audio,
	       sizeof(struct audio_control));
...
static void load_user_wb_setting(void)
{
	...
	memcpy((void *)&colortemp_table[COLOR_TEMP_COLD],
	       (void *)&g_wb_setting.cold, sizeof(vpu_colortemp_table_t));

	memcpy((void *)&colortemp_table[COLOR_TEMP_STD],
	       (void *)&g_wb_setting.standard, sizeof(vpu_colortemp_table_t));

	memcpy((void *)&colortemp_table[COLOR_TEMP_WARM],
	       (void *)&g_wb_setting.warm, sizeof(vpu_colortemp_table_t));

	memcpy((void *)&colortemp_table[COLOR_TEMP_USER],
	       (void *)&g_wb_setting.user, sizeof(vpu_colortemp_table_t));
...
void pq_load_spi_data(void)
{
	...
	spi_flash_read(get_spi_flash_device(0), PQ_PARAM_AREA_BASE_OFFSET,
		       sizeof(vpu_config_t), des);
...
static unsigned int handle_setting_cmd(unsigned char *s, int *rets)
{
	...
	case VPU_CMD_WB_VALUE:
		...
		wbs = (white_balance_setting_t *)
		    malloc(sizeof(white_balance_setting_t));
...
static int setting_task_handle(int task_id, void *param)
{
	...
	if (plist != NULL) {
		...
		if (clist != NULL) {
			...
			unsigned char *cmd =
			    (unsigned char *)(clist->cmd_data.data);
			...
			if (cmd != NULL) {
				...
				if (rcmd_len > 0) {
					...
					SendReturn(setting_task_id,
						   clist->cmd_data.cmd_owner,
						   *cmd, (int *)params);
...
void read_pq_by_id(unsigned int id, unsigned char *buf)
{
	...
	rd_first_end_id =
	    (FBC_USER_START - PQ_BINARY_START) / PQ_BINARY_UNIT_SIZE;
	...
	spi_flash_read(get_spi_flash_device(0),
		       rd_pq_bin_start + rd_real_id * PQ_BINARY_UNIT_SIZE,
...
int load_pq_by_id(unsigned int id)
{
	...
	if (buf != NULL) {
		...
		if (isPorjectIDValid(id) == 0) {
			...
			bin_pq_crc =
			    *((unsigned int *)&buf[PQ_BINARY_UNIT_SIZE - 4]);

			if (cal_pq_crc != bin_pq_crc) {
				printf("pq binary checksum error");
				printf
				    ("(%d, 0x%08X, 0x%08X), load pq failed!\n",
	...
	if (buf != NULL) {
		...
		if (isPorjectIDValid(id) == 0) {
			...
			if (def_pq_crc != bin_pq_crc) {
				...
				printf
				("id(%d) pq crc error!!!, lets load it's pq!\n",
				 id);
...
void set_nature_lihgt_en(unsigned int on_off)
{
	...
	spi_flash_random_write(get_spi_flash_device(0), USER_SETTING_START,
			       sizeof(struct user_setting_save),
			       &g_user_setting_save);
...
int save_task_handle(int task_id, void *param)
{
	...
	if (1 == g_user_setting.change_flag) {
		...
		memcpy((void *)&g_user_setting_save.pq,
		       (void *)g_user_setting.pq, sizeof(vpu_fac_pq_t));

		memcpy((void *)&g_user_setting_save.audio,
		       (void *)g_user_setting.audio,
		       sizeof(struct audio_control));
...
void init_setting_task(void)
{
	...
	spi_flash_read(get_spi_flash_device(0), USER_SETTING_START,
		       sizeof(struct user_setting_save),
		       (void *)&g_user_setting_save);

	spi_flash_read(get_spi_flash_device(0), USER_WB_SETTING_START,
		       sizeof(struct wb_setting), (void *)&g_wb_setting);

	spi_flash_read(get_spi_flash_device(0), USER_PIC_SETTING_START,
		       PICMOD_MAX * sizeof(vpu_picmod_table_t),
		       (void *)&picmod_table);

	memset((void *)&gCriData, 0, sizeof(struct cri_data));

	spi_flash_read(get_spi_flash_device(0), USER_CRI_DATA_START,
		       sizeof(struct cri_data), (void *)&gCriData);

	if (g_user_setting_save.system.version != system_default.version) {

		printf
		    ("current version is 0x%08x, default version is 0x%08x\n",
		     g_user_setting_save.system.version,
	...
	setting_task_id =
	    RegisterTask(setting_task_handle, NULL, 0, TASK_PRIORITY_USER);

	if (setting_task_id > 0) {

		RegisterCmd(&setting_list, setting_task_id,
			    INPUT_CEC | INPUT_UART_HOST,
			    check_cmd_is_supported, handle_setting_cmd);
