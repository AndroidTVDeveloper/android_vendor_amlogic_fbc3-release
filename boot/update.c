	...
	do {
		...
		if (rc_pkg_state == update_ctrl->flag) {
			...
			if (c == '\n' || c == '\r' || i == MAX_CMD_LENGTH) {
				if (i == MAX_CMD_LENGTH) {
					update_ctrl->buf[MAX_CMD_LENGTH - 1] =
					    '\0';
				}
				update_ctrl->buf[i] = '\0';
...
int move_image(struct spi_flash *flash, unsigned s_offs, unsigned b_offs,
	       unsigned size)
{
...
int handle_cmd(update_ctrl_t *update_ctrl)
{
	...
	if (update_ctrl->argv[0]
	    && (!strcmp(CONFIG_REBOOT_CMD, update_ctrl->argv[0])
		|| !strcmp(CONFIG_EXIT_CMD, update_ctrl->argv[0]))) {
		update_ctrl->serial_dev->puts("\nexit upgrade!\n");
		return 1;
	} else if (update_ctrl->argv[0]
		   && (!strcmp(CONFIG_UPDATE_CMD, update_ctrl->argv[0]))) {
		if (update_ctrl->argv[1] && update_ctrl->argv[2]) {
			update_ctrl->s_offs =
			    strtoul(update_ctrl->argv[1], NULL, 16);
			update_ctrl->total =
			    strtoul(update_ctrl->argv[2], NULL, 16);
		} else {
			update_ctrl->serial_dev->puts("\ncmd format error!\n");
...
int handle_restore(struct update_ctrl *ctrl)
{
	...
	return move_image(ctrl->flash_dev, ctrl->b_offs, ctrl->s_offs,
			  ctrl->total);
}
...
int handle_update(struct update_ctrl *ctrl)
{
	...
	for (erase_size = SPI_FLASH_SECTOR_SIZE; erase_size < ctrl->total;
	...
	do {
		...
		printfx("length = %d\n",
			(ctrl->received_cnt >=
			 MAX_BUF_SIZE) ? MAX_BUF_SIZE : ctrl->received_cnt);
		crc =
		    crc32(0, ctrl->buf,
			  (ctrl->received_cnt >=
			   MAX_BUF_SIZE) ? MAX_BUF_SIZE : ctrl->received_cnt);
		if (crc != ctrl->crc) {
			...
			if (ctrl->show_update_msg) {
				sprintf(msg_buf,
					"CRC error!!! calculate crc = 0x%X, receive crc = 0x%X",
					crc, ctrl->crc);
				ctrl->show_update_msg(1, msg_buf);
			}
			return -1;
...
rewrite:
		...
		wlen =
		    spi_flash_write(ctrl->flash_dev,
				    ctrl->s_offs + ctrl->write_spi_cnt, ret,
				    ctrl->buf);
		...
		if (ctrl->show_update_msg) {
			sprintf(msg_buf, "write offset: 0x%X, size: 0x%X",
				ctrl->s_offs, erase_size);
			ctrl->show_update_msg(0, msg_buf);
		}
		printfx
		    ("ctrl->write_spi_cnt = %d",
		     ctrl->write_spi_cnt);

		printfx
		    (", ctrl->total = 0x%x, wlen = 0x%x\n",
		     ctrl->total, wlen);
...
int handle_check(struct update_ctrl *ctrl)
{
	...
	for (s_cmp = 0, offset = ctrl->s_offs, total = ctrl->write_spi_cnt;
	     total > 0; offset += len, total -= len, s_cmp += len) {
...
void init_update_ctrl_t(update_ctrl_t *update_ctrl)
{
	...
	if (REBOOT_FLAG_UPGRADE1 == update_ctrl->channel
	    || REBOOT_FLAG_LITE_UPGRADE1 == update_ctrl->channel) {
		update_ctrl->serial_dev =
		    get_serial_device(CONSOLE_CHANNEL_DEV);
	} else {
